...
class ProductivityDashboard:
    ...
    def _build_settings_tab(self):
        ...
        heading.pack(fill='x')
        ttk.Label(heading, text="Settings", font=("Segoe UI", 14, 'bold')).pack(side='left')
        ttk.Button(heading, text="Save Settings", command=self._save_settings).pack(side='right')
        ...
        for column, (label, variable) in enumerate((
            ...
            ttk.Label(timing, text=label).grid(row=0, column=column, padx=8, pady=(6, 2), sticky='w')
        ...
        self.settings_texts = {}
        self._add_rule_editor(classification, "Building app/process patterns", "building_apps", 0)
        self._add_rule_editor(classification, "Studying app/title patterns", "studying_apps", 1)
        self._add_rule_editor(classification, "Applying site/title patterns", "applying_sites", 2)
        self._add_rule_editor(classification, "Pseudo-productive site/title patterns", "pseudo_productive_sites", 3)
        self._add_rule_editor(blocking, "Blocked domains", "blocked_sites", 0, height=8)
        self._add_rule_editor(blocking, "Blocked executable names", "blocked_apps", 1, height=8)
        ttk.Label(
            container,
    ...
    def _save_settings(self):
        ...
        try:
            ...
            self._load_settings_form()
            messagebox.showinfo("Settings saved", "New tracking and focus sessions will use these rules.")
